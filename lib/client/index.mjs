...
export function apply() {
  ...
  // ---- 渲染 ----
  // 滚动位置保护（三栏通用）：重渲染前后直接恢复 scrollTop 数值——
  // 消息只追加在底部、视口上方内容高度不变，精确恢复 scrollTop 即可保持阅读位置。
  // 不用 scrollIntoView：它会把锚点强制钉到视口顶部，阅读中间位置时内容会跳一个视口高度（实测踩坑）。
  // 贴底（距底 <48px）且内容确实变高时才跟随新消息到底。
  const SCROLL_PIN_THRESHOLD = 48
  const scrollKeep = (el) => {
    const prevTop = el.scrollTop
    const prevHeight = el.scrollHeight
    const nearBottom = prevHeight - prevTop - el.clientHeight < SCROLL_PIN_THRESHOLD
    return () => {
      if (nearBottom && el.scrollHeight > prevHeight) {
        el.scrollTop = el.scrollHeight
      } else {
        el.scrollTop = Math.min(prevTop, Math.max(0, el.scrollHeight - el.clientHeight))
      }
    }
  }
  ...
  const renderTimeline = () => {
    const keep = scrollKeep(timeline)
    timeline.innerHTML = ''
    const room = state.rooms.find((r) => r.id === currentRoomId)
    if (!room) {
