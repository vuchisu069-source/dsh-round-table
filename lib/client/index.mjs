...
export function apply() {
  ...
  // ---- 渲染 ----
  // 滚动位置保护（三栏通用）：贴底（距底 <48px）跟随到底；上翻阅读时锚定首个可见项，
  // 重渲染后 scrollIntoView 复位——防止轮询/SSE 触发重渲染时滚动位置被重置（严重体验 bug）
  const SCROLL_PIN_THRESHOLD = 48
  const scrollKeep = (el, keyOf) => {
    const nearBottom = el.scrollHeight - el.scrollTop - el.clientHeight < SCROLL_PIN_THRESHOLD
    let anchor = null
    if (!nearBottom) {
      for (const child of el.children) {
        const rect = child.getBoundingClientRect()
        if (rect.bottom >= 0) { const k = keyOf(child); if (k !== null) { anchor = k; break } }
      }
    }
    return () => {
      if (nearBottom) { el.scrollTop = el.scrollHeight; return }
      if (anchor === null) return
      for (const child of el.children) {
        if (keyOf(child) === anchor) { child.scrollIntoView({ block: 'start' }); return }
      }
    }
  }
  ...
  const renderLeft = () => {
    const keep = scrollKeep(left, (c) => c.dataset?.rtKey ?? null)
    left.innerHTML = ''
    ...
    for (const room of state.rooms) {
      const row = document.createElement('div')
      row.dataset.rtKey = `room:${room.id}`
    ...
    for (const role of state.roles) {
      const row = document.createElement('div')
      row.dataset.rtKey = `role:${role.id}`
  ...
  // 时间线滚动策略：贴底（距底 <48px）才跟随到底；用户上翻阅读历史时保持锚点不动
  // （此前每次渲染强制滚到底，轮询 3s 一刷就把阅读位置拽回底部——严重体验 bug）
  const renderTimeline = () => {
    const keep = scrollKeep(timeline, (c) => c.dataset?.msgId ?? null)
    timeline.innerHTML = ''
    const room = state.rooms.find((r) => r.id === currentRoomId)
    if (!room) {
  ...
  const renderRight = () => {
    const keep = scrollKeep(right, (c) => c.dataset?.rtKey ?? null)
    right.innerHTML = ''
    const room = state.rooms.find((r) => r.id === currentRoomId)
    if (!room) {
      // 空态引导：没有房间时右栏不空白
    ...
    for (const member of room.members) {
      ...
      const row = document.createElement('div')
      row.dataset.rtKey = `member:${member.id}`
    ...
    if (room.summaries.length > 0) {
      ...
      for (const s of room.summaries) {
        ...
        card.dataset.rtKey = `summary:${s.id}`
        card.textContent = s.text
        card.style.cssText = 'font-size:11px; color:#c9d1de; background:rgba(255,255,255,.05); border-radius:8px; padding:6px 8px; margin-bottom:6px; white-space:pre-wrap; word-break:break-word;'
