...
export function apply() {
  ...
  const renderTimeline = () => {
    const wasNearBottom = timeline.scrollHeight - timeline.scrollTop - timeline.clientHeight < SCROLL_PIN_THRESHOLD
    let anchorId = null
    if (!wasNearBottom) {
      for (const child of timeline.children) {
        const rect = child.getBoundingClientRect()
        if (rect.bottom >= 0 && typeof child.dataset?.msgId === 'string') { anchorId = child.dataset.msgId; break }
      }
    }
    timeline.innerHTML = ''
    const room = state.rooms.find((r) => r.id === currentRoomId)
    ...
    if (wasNearBottom) {
      ...
      for (const child of timeline.children) {
        if (child.dataset?.msgId === anchorId) {
